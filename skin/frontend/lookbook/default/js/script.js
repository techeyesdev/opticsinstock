...
jQuery(window).load(function() {
  ...
  /* Coin Slider */
	...
	function replacingClass () {
		...
		if (jQuery(document.body).width() > 977){ //Desktop + Extra Large
			mobile_menu('reset');
		}
	}
	replacingClass();
	...
	/* header type 2 */
	if(jQuery('.header-2').length){
		jQuery('.header-2').css('min-height', jQuery('.header-2 h1.logo').height());
	}
...
jQuery(document).ready(function(){
	...
	jQuery(window).scroll(function() {
		if(jQuery(this).scrollTop() >= 155 ){
			if(!jQuery('#floating-header').hasClass('active')){
				jQuery('#floating-header').addClass('active').slideDown('fast');
			}
		}
		if(jQuery(this).scrollTop() < 155 ){
			if(jQuery('#floating-header').hasClass('active')){
